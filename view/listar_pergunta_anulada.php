...
{
    $data = $_GET['search'];
    $sql = "SELECT * FROM tb_perguntas WHERE (id_pergunta LIKE '%$data%'  or pergunta LIKE '%$data%' or resposta LIKE '%$data%') and status_pergunta = 'EXC' ORDER BY id_pergunta DESC";
}
else
{
    $sql ="SELECT * FROM tb_perguntas WHERE status_pergunta = 'EXC' ORDER BY id_pergunta";
}
